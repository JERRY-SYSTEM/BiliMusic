...
class _CachedCoverImageState extends State<CachedCoverImage> {
  ...
  void didChangeDependencies() {
    ...
    // A modal sheet/page can place this widget outside the screen during its
    // first layout, then move it into view without changing the scroll
    // position. Listening to the route animation makes visibility get checked
    // again as the sheet enters instead of waiting for the user's first drag.
    final nextRouteAnimation = ModalRoute.of(context)?.animation;
    if (!identical(nextRouteAnimation, _routeAnimation)) {
      _routeAnimation?.removeListener(_scheduleVisibilityCheck);
      _routeAnimation = nextRouteAnimation;
      _routeAnimation?.addListener(_scheduleVisibilityCheck);
    }
