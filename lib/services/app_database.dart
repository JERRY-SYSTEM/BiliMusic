...
class AppDatabase {
  ...
  static const String _autoCoverMatchMissesSchema =
      'CREATE TABLE IF NOT EXISTS auto_cover_match_misses '
  ...
  static Future<Database> _open() async {
    try {
      ...
      return await factory.openDatabase(path, options: OpenDatabaseOptions(
        ...
        // This table was introduced without a database-version bump in an
        // earlier build. Some installed databases therefore report the
        // current version while still missing it. Repair that state every
        // time the database opens instead of relying solely on onUpgrade.
        onOpen: (db) => db.execute(_autoCoverMatchMissesSchema),
        onCreate: (db, version) async {
          ...
          await db.execute('CREATE TABLE auto_cover_match_misses (track_id TEXT PRIMARY KEY)');
        ...
        onUpgrade: (db, oldVersion, newVersion) async {
          for (final table in [
            ...
            'playback_state', 'cover_cache', 'auto_cover_match_misses',
          ]) {
  ...
  static Future<void> ensureAutoCoverMatchMissesTable() async {
    await (await instance).execute(_autoCoverMatchMissesSchema);
  }
